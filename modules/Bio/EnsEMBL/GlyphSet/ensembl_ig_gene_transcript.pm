package Bio::EnsEMBL::GlyphSet::ensembl_ig_gene_transcript;
use strict;
...
sub my_label {
  my $self = shift;
  return $self->{'config'}->{'_draw_single_Transcript'} || $self->{'config'}->{'geneid'} || "Ig genes.";
}

sub colours {
  my $self = shift;
  my $Config = $self->{'config'};
  return $Config->get('ensembl_ig_gene_transcript','colours');
}

sub features {
  my ($self) = @_;
  my $track = 'ensembl_ig_gene_transcript';
  my $slice = $self->{'container'};
  my $sp = $self->{'_config_file_name_'};
  my @analyses = ( 'ensembl_ig_gene' );
...
sub href {
  ...
  return ( $self->{'config'}->get('ensembl_ig_gene_transcript','_href_only') eq '#tid' && exists $highlights{lc($gene->stable_id())} ) ?
    "#$tid" : 
...
sub gene_href {
  ...
  my $script_name = $ENV{'ENSEMBL_SCRIPT'} eq 'genesnpview' ? 'genesnpview' : 'geneview';
  return ( $self->{'config'}->get('ensembl_ig_gene_transcript','_href_only') eq '#gid' && exists $highlights{lc($gene->stable_id)} ) ?
    "#$gid" :
    qq(/@{[$self->{container}{_config_file_name_}]}/$script_name?gene=$gid);
...
sub ajax_zmenu {
  ...
  my $zmenu = EnsEMBL::Web::Interface::ZMenu->new( (  
                                     ...
                                     type  => 'ensembl_ig_gene_transcript',
...
sub static_zmenu { 
  ...
  my $zmenu = {
    'caption'                       => "Ig ene: ".$transcript->biotype,
    "00:$id"			=> "",
