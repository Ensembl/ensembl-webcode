package Bio::EnsEMBL::GlyphSet::fugu_protein_transcript;
use strict;
use vars qw(@ISA);
...
sub my_label {
    return 'T. rubripes trans.';
}

sub colours {
    my $self = shift;
    my $Config = $self->{'config'}->get('fugu_protein_transcript','colours');
}
...
sub colour {
  ...
  return (@{$colours->{'fugu_protein'}}, $highlight);
}
...
sub href {
  ...
  return ( $self->{'config'}->get('fugu_protein_transcript','_href_only') eq '#tid' && exists $highlights{lc($gene->stable_id())} ) ?
        "#$tid" : 
...
sub gene_href {
  ...
  my $gid = $gene->stable_id();
  return ( $self->{'config'}->get('fugu_protein_transcript','_href_only') eq '#gid' && exists $highlights{lc($gid)} ) ?
...
sub features {
  my ($self) = @_;
  my $track = 'fugu_protein_transcript';
  my $slice = $self->{'container'};
  my @genes;
  foreach my $analysis( 'fugu_protein' ){
warn "... $analysis ...";
...
sub legend {
  my ($self, $colours) = @_;
  return ('fugu_protein', 1000, 
    [ 'T. rubripes genes' => $colours->{'fugu_protein'}[0], ]
  );
}

sub error_track_name { return 'T. rubripes proteins'; }
