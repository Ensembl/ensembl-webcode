...
sub colours {
    my $self = shift;
    my $Config = $self->{'config'}->get('human_protein_transcript','colours');
}
...
sub colour {
  ...
  my $highlight = undef;
  my $colour = $colours->{'human_protein'}[0];
...
sub href {
  ...
  return ( $self->{'config'}->get('human_protein_transcript','_href_only') eq '#tid' && exists $highlights{lc($gene->stable_id())} ) ?
        "#$tid" : 
...
sub gene_href {
  ...
  my $gid = $gene->stable_id();
  return ( $self->{'config'}->get('human_protein_transcript','_href_only') eq '#gid' && exists $highlights{lc($gid)} ) ?
...
sub features {
  my ($self) = @_;
  my $track = 'human_protein_transcript';
  my $slice = $self->{'container'};
  my @genes;
  foreach my $analysis( 'human_protein' ){
...
sub legend {
  my ($self, $colours) = @_;
  return ('human_protein', 1000, 
    [ 'Human genes' => $colours->{'human_protein'}[0], ]
  );
}
