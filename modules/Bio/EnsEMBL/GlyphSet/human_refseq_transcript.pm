package Bio::EnsEMBL::GlyphSet::human_refseq_transcript;
use strict;
use vars qw(@ISA);
...
sub colours {
    my $self = shift;
    my $Config = $self->{'config'}->get('human_refseq_transcript','colours');
}
...
sub href {
  ...
  return ( $self->{'config'}->get('human_refseq_transcript','_href_only') eq '#tid' && exists $highlights{lc($gene->stable_id())} ) ?
        "#$tid" : 
...
sub gene_href {
  ...
  my $gid = $gene->stable_id();
  return ( $self->{'config'}->get('human_refseq_transcript','_href_only') eq '#gid' && exists $highlights{lc($gid)} ) ?
...
sub features {
  my ($self) = @_;
  my $track = 'human_refseq_transcript';
  my $slice = $self->{'container'};
  my @genes;
  foreach my $analysis( 'human_refseq' ){
...
sub legend {
  my ($self, $colours) = @_;
  return ('human_refseq', 1000, 
    [ 'Human refseq' => $colours->{'refseq'}[0], ]
  );
}

sub error_track_name { return 'Human refseq'; }
