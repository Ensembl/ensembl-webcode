...
sub process {
  ...
  if (!$format_info) {
    ...
    unless ($error) {
      ...
      if (($hub->param('align') && lc($format) ne 'rtf')
          || (ref($component) =~ /Homoeolog|Paralog/ && $in_bioperl && lc($format) ne 'fasta')) {
  ...
  if ($error) {
    ...
    push @core_params, qw(export_action data_type data_action component align align_type clusterset_id filtered_sets g1 hom_id node strain);
