...
sub content {
  ...
  foreach my $varif_id (grep $_ eq $hub->param('vf'), keys %mappings) {
    ...
    foreach my $transcript_data (@{$mappings{$varif_id}{'transcript_vari'}}) {
      ...
      my $trans_length_label = ($transcript_data->{'cdna_start'}) ? qq{<span class="small"> (out of $trans_length)</span>} : '';
      my $cds_length_label   = ($transcript_data->{'cds_start'}) ? qq{<span class="small"> (out of $cds_length)</span>} : '';
      my $pr_length_label    = ($transcript_data->{'translation_start'}) ? qq{<span class="small"> (out of $pr_length)</span>} : '';
      ...
      my $row = {
        ...
        type      => $type,
        trans_pos => $self->_sort_start_end($transcript_data->{'cdna_start'},        $transcript_data->{'cdna_end'}) . $trans_length_label . $cdna_overlap,
        cds_pos   => $self->_sort_start_end($transcript_data->{'cds_start'},         $transcript_data->{'cds_end'}) . $cds_length_label . $cds_overlap,
        prot_pos  => $self->_sort_start_end($transcript_data->{'translation_start'}, $transcript_data->{'translation_end'}) . $pr_length_label . $pr_overlap,
    ...
    for my $rfv (@{ $vf_obj->get_all_RegulatoryFeatureVariations }) {
      ...
      for my $rf (@$rfs) {
        ...
        my $var_pos = $var_start - $rf->seq_region_start + 1;
        my $reg_length = $rf->seq_region_end - $rf->seq_region_start + 1;
        my $reg_length_label = qq{<span class="small"> (out of $reg_length)</span>};
        ...
        for my $rfva (@{ $rfv->get_all_alternate_RegulatoryFeatureVariationAlleles }) {
          ...
          my $row = {
            ...
            allele    => $r_allele,
            type      => $type || '-',
            coverage  => $var_pos.$reg_length_label.$regulation_overlap
          };
    ...
    for my $mfv (@{ $vf_obj->get_all_MotifFeatureVariations }) {
      ...
      my $motif_length_label = qq{<span class="small"> (out of $motif_length)</span>};
      ...
      for my $mfva (@{ $mfv->get_all_alternate_MotifFeatureVariationAlleles }) {
        ...
        my $row = {
          ...
          type     => $type,
          matrix   => $matrix_url,
          pos      => $mfva->motif_start.$motif_length_label.$motif_overlap,
