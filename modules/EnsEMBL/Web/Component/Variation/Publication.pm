...
sub table_data { 
  ...
  foreach my $cit (@$citation_data) { 
    ...
    my $row = {
	  ...
	  ucsc   => defined $cit->ucsc_id() ? "<a class=\"_ht\" href=\"" . $ucsc_url . $object->name() ."&pubsFilterExtId=". $cit->ucsc_id() . "\" title=\"View in UCSC\"><img src=\"/i/val/ucsc_logo_sm.png\" style=\"border-radius:5px;border:1px solid #000;padding:1px;background-color:#FFF\" alt=\"View\" /><span class=\"hidden export\">" . $ucsc_url . $object->name() ."&pubsFilterExtId=". $cit->ucsc_id() . "</span></a>" : undef 
    };
