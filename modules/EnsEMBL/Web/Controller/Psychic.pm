...
sub psy_redir {
    ...
    if ($do_redirect) {
        # warn "Psychic redirecting to $url";
        return $self->redirect($url);
    } else {
        # warn "Psychic not redirecting to $url";
        return $url;
    }
}
...
sub psychic {
  ...
  return $self->psy_redir("//www.ebi.ac.uk/ebisearch/search.ebi?db=allebi&query=$query")  if $dest_site eq 'ebi';
  return $self->psy_redir("//www.sanger.ac.uk/search?db=allsanger&t=$query")              if $dest_site eq 'sanger';
  return $self->psy_redir("//www.ensemblgenomes.org/search/eg/$query")                    if $dest_site eq 'ensembl_genomes';
  ...
  if ($species || $query_species ) {
    ...
    if ($query =~ /^rs\d+$/) {
      ...
      return $self->psy_redir($site.$hub->url({
