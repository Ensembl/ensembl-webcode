...
## Now set values set by code...
    $self->_set_align(   $arg_ref->{'align'}   ) if exists $arg_ref->{'align'  };
    $self->_set_valign(  $arg_ref->{'valign'}  ) if exists $arg_ref->{'valign' };
    $self->_set_bold(    $arg_ref->{'bold'}    ) if exists $arg_ref->{'bold'   };
    $self->_set_italic(  $arg_ref->{'italic'}  ) if exists $arg_ref->{'italic' };
