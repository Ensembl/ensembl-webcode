...
sub search_SEQUENCE {
  ...
  $self->_fetch_results( 
    [ 'core', 'Sequence',
      ...
      "select sr.name, cs.name, sr.seq_region_id, 1, sr.length from seq_region as sr, coord_system as cs where cs.coord_system_id = sr.coord_system_id and sr.name [[COMP]] '[[KEY]]'" ],
    [ 'core', 'Sequence',
