...
# Make sure a DAS source is available on all the views it is suitable for,
# and switched on for the 'current view' (if it is suitable)
sub configure_das_views {
  ...
  # positional sources are attached to images. The DASConfig "on" method gives
  # a combined list of suitable views (e.g. Gene/ExternalData) and images (e.g
  # contigview).
  
  # There are two steps: making the source available on all the views/images it
  # is -suitable for-, and switching the source on for the current view/image.
  # A source will never be made available or switched on for a view it is not
  # suitable for.
  
  # For all the views the source is available on... (auto-generated by inspecting the source)
  for ( @{ $das->on } ) {
    my ( $type, $action ) = split /\//, $_;
    
    # Gene/protein DAS is text, and doesnt use images...
    if ( $action && $action eq 'ExternalData' ) {
      if ( my $vc = $self->getViewConfig( $type, $action ) ) {
        # Allow the source to be switched on from this view
        if ( !$vc->is_option( $das->logic_name ) ) {
          $vc->_set_defaults( $das->logic_name, 'no'  );
        }
        # And if this is the view we are currently on, actually switch it on
        if ( $type eq $this_type && $action eq $this_action ) {
          $vc->set( $das->logic_name, 'yes' );
        }
      }
    }
    
    elsif ( my $ic = $self->getImageConfig( $type, $type ) ) {
      my $n = $ic->get_node( 'das_' . $das->logic_name );
      # Allow the source to be switched on in this image
      if( !$n ) {
        $n = $ic->tree->create_node( 'das_' . $das->logic_name, { 'display' => 'off' } );
      }
      # And if the current view has this image, actually switch it on
      # This logic looks a bit funny because of a degree of disconnection
      # between image configs and view configs
      if ( $this_ics{$type} ) {
