...
use EnsEMBL::Web::Wizard::Connection::Option;
use EnsEMBL::Web::Wizard::Node;
use EnsEMBL::Web::Wizard::Node::Final;
...
sub new {
  ### c
  ### Creates a new inside-out Wizard object. This object controls
  ### a number of Node objects to create a wizard interface. A
  ### Wizard object only controls the flow of the linked wizard nodes, 
  ...
  ### {{EnsEMBL::Web::Wizard::Node}} objects.
  ###
  ### The wizard collections information from each node, and 
...
sub backward_connection {
  ...
  if ($previous) {
    ...
    if (!$backward_connection) {
      my $connection = EnsEMBL::Web::Wizard::Connection->new(( type => 'link', 
...
sub add_connection {
  ...
  if ($params{type} eq 'option') {
    $connection = EnsEMBL::Web::Wizard::Connection::Option->new();
  } else {
    $connection = EnsEMBL::Web::Wizard::Connection->new();
  }
  if ($connection) {
...
sub render_error_message {
  my ($self, $message) = @_;
  return "<h1>Error from Wizard:</h1>$message";
}
