...
sub start {
  ...
  my $text = qq(
<ul class="spaced">
<li><a href="/common/user_data?node_name=file_upload;previous_node=start;source=local">In a file on my local machine</a></li>
<li>On the internet:
<ul>
  <li><a href="/common/user_data?node_name=file_info;previous_node=start;source=url">In a file on a web server</a> (URL-based data)</li>
  <li><a href="/common/user_data?node_name=das_servers;data_type=UserDAS;DASdomain=das.ensembl.org">On an existing DAS server</a></li>
  <li><a href="/info/using/external_data/das/das_server.html">I would like help in setting up my own DAS server</a></li>);
  if ($ENV{'ENSEMBL_USER_ID'}) {
    $text .= qq(
<li><a href="/common/user_data?node_name=account">I have already uploaded the data to my Ensembl user account</li>);
  }
  $text .= qq(</ul>
</li>
</ul>
