...
sub distro_logic {
  ...
  if ($self->object->param('distribution') eq 'global') {
    $parameter->{'wizard_next'} = 'guide_global';
  }
  else {
    $parameter->{'wizard_next'} = 'guide_local';
  }
  return $parameter;
}
...
sub guide_global {
  ...
  my $text = qq(<p>If you wish to view your data in ContigView, we recommend uploading it to an Ensembl user account, otherwise performance may be slow.</p>);
  ...
  $self->add_element(( type => 'RadioButton', name => 'method', id => 'method_1', value => 'upload', label => 'Upload data to my Ensembl account', checked => 'checked'));
  $self->add_element(( type => 'RadioButton', name => 'method', id => 'method_2', value => 'url', label => 'Attach data via URL', checked => 'checked'));
}
...
sub guide_local {
  ...
  my $text = qq(<p>Storing your data in your Ensembl account makes it easy to reuse and may improve performance, 
...
sub file_upload {
  ...
  $self->add_element(( type => 'File', name => 'upload', label => 'Upload file' ));
