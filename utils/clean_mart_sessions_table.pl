...
my $dsn = sprintf('DBI:mysql:database=%s;host=%s;port=%s', $sd->ENSEMBL_USERDB_NAME, $sd->ENSEMBL_USERDB_HOST, $sd->multidb->{'DATABASE_SESSION'}{'PORT'} || $sd->ENSEMBL_USERDB_PORT);
